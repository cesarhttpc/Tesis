...
\begin{figure}
    ...
    \label{Fig. 3.2.2.05}}
    \caption{Distribución posterior (azul) y distribución a priori (verde) para el parámetros $g$ y $b$ del modelo gravitatorio. Los valores verdaderos se marcan en negro.}
...
En el modelo gravitacional tenemos un ejemplo de simulación del problema inverso donde las unidades de los parámetros no difieren significativamente en ordenes de magnitud. En secciones posteriores se explica el concepto de regularización de las unidades, donde la metodología aproximada para modelos cuyos ordenes de magnitud difieren significativamente tienen sesgo en la estimación de la distribución posterior. Dicho lo anterior, siguiendo la metodología ordinaria para la resolución del problema inverso en el modelo logístico, con la base de datos simuladas del mismo considerando $\theta_1 = 0.001$ y $\theta_2 = 1000$ con un ruido gaussiano aditivo de desviación estándar $\sigma = 30$ graficado en la Fig. \ref{Fig. 3.2.log.muestra}.
...
Posteriormente, definamos las constantes de las distribuciones gamma a priori en (\ref{3.2.1.03}). Seleccionemos las constantes semejantes a los valores verdaderos, cuyas gráficas se muestran en la Fig. \ref{Fig. 3.2.log.priori}
...
\begin{figure}
    ...
    \caption{Distribución posterior (azul) y distribución a priori (verde) para el parámetros $\theta_1$ y $\theta_2$ del modelo logístico. Los valores verdaderos se marcan en negro.}
...
Para el modelo epidemiólogico tomamos una muestra de $n =10$ observaciones de infectados en una población cerrada a lo largo del tiempo. Al igual que los modelos previos, dichas observaciones son simulaciones del forward map con ruido gaussiano. Tomando una muestra de $n = 10$ con parámetros $\beta = 0.009$, $\gamma = 0.5$ y ruido con desviación estándar de $\sigma = 10$ y dando las condiciones iniciales por conocidas con $I(0) = 50$, $S(0)= 500$, $R(0)=5$. En la Fig. \ref{Fig. SIR_01} vemos la gráfica de la muestra de infectados, notamos que tienen un rápido crecimiento para después decaer suavemente.
...
\begin{figure}[h]
    ...
    \label{Fig. SIR_05}}
    \caption{Distribución posterior (azul) y distribución a priori (verde) para el parámetros $\beta$ y $\gamma$ del modelo SIR. Los valores verdaderos se marcan en negro. }
    \label{Fig. SIR.theta}
\end{figure}
...
\subsection{Construcción de la regularización de unidades}
...
En el análisis bayesiano del problema inverso del modelo gravitatorio se obtuvo la distribución posterior con el forward map ordinario. Esta vez, el análisis bayesiano se hará con el forward map aproximados para cada par $k$ y $M$ de las aproximaciones propuestas. Posteriormente, las distribuciones aproximadas $\tilde{\pi}^{k}_M(g,b)$ las cotejaremos con su análogo ordinario, $\pi(g,b)$ de forma que sea visible o no la convergencia al análogo ordinario.

Para empezar, tomemos la aproximación del forward map con 3 vecinos cercanos $(k= 3)$. Luego, estimamos la distribución posterior conjunta para cada rejilla $(M = 10, 15, 30, 50)$ haciendo MCMC con el t-walk \citep{christen2010general}. Posteriormente se toman las distribuciones marginales para cada parámetro $g,b$. Al ser un estudio heurístico, el grado de aproximación se hace tras simple comparación de la distribución marginal aproximada  para cada parámetro, $\tilde{\pi}^{k}_M(g)$ con $\pi(g)$, y comparación de $\tilde{\pi}^{k}_M(b)$ con $\pi(b)$.
...
Notemos del análisis previo que de manera general las distribuciones posteriores aproximadas $\tilde{\pi}^{k}_M(g,b)$ mejoran a medida que la malla se hace más fina. Luego, la mejor aproximación se obtiene con la malla más fina que discretiza el espacio parametral en 50x50 puntos. Queda por definir que cantidad de vecinos hace la mejor aproximación. Nótese que para 8 vecinos (k = 8) no ajusta para el parámetro $g$ lo que descarta esta elección. Finalmente, para 3 y 5 vecinos tenemos distribuciones bastante similares por lo que la elección es indiferente. Así, seleccionamos el forward map aproximado con 3 vecinos en una malla 50x50.

Para saber la factibilidad del método se considera al tiempo de ejecución, que es una variable tangible y de fácil acceso. En la tabla \ref{tabla_01} se muestran los tiempos de ejecución mencionados para cada aproximación del forward map considerada. Por otro lado, el tiempo de ejecución del método ordinario para el problema inverso en el modelo gravitacional es de \textbf{5 min 57 s}. Esto es considerando un MCMC de $T = 600,000$ iteraciones y un burn in de $20,000$, al igual que cualquier MCMC considerado en esta tesis.
...
Es de interés analizar la metodología aproximada propuesta para otros modelos. La experimentación con el modelo logístico nos permite dilucidar el comportamiento de la metodología en cuestión. Al igual que el modelo gravitacional, aquí realizamos los mismos experimentos con $k = 3,5,8$ y una resolución de malla de $M = 10, 15, 30, 50$. En todos los casos usamos el t-walk con $T = 600,000$ iteraciones y un burn in de $20,000$.
...
El tiempo de ejecución del método ordinario para el problema inverso en el modelo logístico es de \textbf{20 min 58 s}. Mientras que en la tabla \ref{tabla_02} tenemos los tiempos de ejecución del modelo logístico con forward map aproximado para cada caso considerado.
...
En el modelo epidemiológico SIR, el cual sabemos que su problema directo no tiene solución analítica, tiene relevancia en las diferencias de los ordenes de magnitud para sus parámetros $\alpha,\beta$, que es un experimento de merito para la metodología aproximada propuesta, de regularizar las unidades.


De igual forma que las aproximaciones en modelos precedentes, en este caso se hacen repetidas distribuciones posteriores aproximadas con distintos forward map aproximados. Esto es tomar forward maps que consideren 3, 5, 8 vecinos cercanos con resolución de malla M = 10, 15, 30, 50. De igual forma que los casos anteriores, se usa el t-walk con $T = 600,000$ iteraciones y un burn in de $20,000$ iteraciones.
...
El tiempo de ejecución del método ordinario para el problema inverso en el modelo SIR es de \textbf{14 min 05 seg}. Mientras que los distintos tiempos de ejecución para las posteriores aproximadas se tabulan a continuación.
